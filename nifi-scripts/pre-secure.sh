...
echo '' >> ${NIFI_REGISTRY_HOME}/conf/nifi-registry.properties
echo 'nifi.registry.security.identity.mapping.pattern.dn2=^CN=(.*?), .*$' >> ${NIFI_REGISTRY_HOME}/conf/nifi-registry.properties
echo 'nifi.registry.security.identity.mapping.value.dn2=$1' >> ${NIFI_REGISTRY_HOME}/conf/nifi-registry.properties
...
export TRUSTSTORE_PASSWORD=$(echo "$TRUSTSTORE_PASSWORD" | sed -e "s|&|\\\\&|g")
export KEYSTORE_PASSWORD=$(echo "$KEYSTORE_PASSWORD" | sed -e "s|&|\\\\&|g")
export KEY_PASSWORD=$(echo "$KEY_PASSWORD" | sed -e "s|&|\\\\&|g")
